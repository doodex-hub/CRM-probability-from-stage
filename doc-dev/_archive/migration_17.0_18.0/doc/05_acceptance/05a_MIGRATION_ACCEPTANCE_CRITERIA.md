...
Then nilai tersimpan sebagai `ir.config_parameter` key `crm.manual.compute.probability`, global (bukan per-company)
...
Then field "Probability" (widget float, suffix "%") TERLIHAT, ditempatkan tepat sebelum field "Won"
...
Then setting tersebut muncul sebagai satu baris `<setting>` tambahan di block kedua (bersama "Assign salespersons into multiple Sales Teams") — **posisi tampil PERSIS setelah setting mana** boleh berbeda dari 17.0 (lihat DIFF-04, kosmetik, bukan kegagalan AC)
...
**AC-02-04** (verifies `BSL-006`, MF-02)
...
When PLS re-compute jalan untuk opportunity itu (`_compute_probabilities` dipanggil — mis. lewat cron/action "Update Probabilities")
Then `automated_probability` diupdate ke hasil PLS (Naive Bayes), TAPI `probability` di-set ke `stage_id.probability` (60) — BUKAN ke `automated_probability` — karena `was_automated` False
...
Then `probability` di-set ke `automated_probability` hasil PLS (mengikuti behavior asli/default)

**AC-02-06** (verifies `BSL-005`, MF-03)
...
Then `is_automated_probability` opportunity itu **TIDAK otomatis ter-recompute** — tetap nilai lama sampai `probability`/`automated_probability` berubah lewat jalur lain (bukan bug baru, dipertahankan dari 17.0)
...
Then `revenue_probability` di-recompute ulang (karena ada di `@api.depends`) TAPI hasilnya TIDAK berubah nilainya — overhead recompute yang tidak perlu, dipertahankan dari 17.0, bukan bug yang mempengaruhi correctness
...
**AC-04-02** (verifies `BSL-016`, MF-01)
