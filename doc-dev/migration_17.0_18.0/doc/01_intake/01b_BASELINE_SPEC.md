...
- `[BSL-001]` `[NO-SPEC]` (ref: —) Admin membuka Settings → CRM, mencentang/uncheck field `crm_manual_compute_probability`. Field ini tersimpan sebagai `ir.config_parameter` key `crm.manual.compute.probability` — bukan per-company, satu nilai global untuk seluruh instance. **Koreksi (2026-08-24, ditemukan Step 9 lewat test asli yang gagal, `test_toggle_saves_config_parameter`):** klaim awal "tersimpan sebagai string `True`/`False` simetris" TIDAK akurat. Odoo core `set_param()` MENGHAPUS key sepenuhnya kalau diberi value falsy (perilaku umum `ir.config_parameter`, bukan spesifik modul ini) — jadi ON tersimpan sebagai string `'True'`, tapi OFF berarti key TIDAK ADA sama sekali, dan `get_param(key, False)` mengembalikan default Python `False` (bool), bukan string `'False'`. Asimetri tipe ini konsisten dengan cara `_compute_show_probability`/`_compute_is_automated_probability` membaca param (selalu pakai `get_param(key, False)` dengan default `False`), jadi tidak mempengaruhi behavior modul — cuma klaim representasi storage di baseline spec ini yang perlu dikoreksi.
